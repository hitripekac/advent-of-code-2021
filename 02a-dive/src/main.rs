...
fn main() -> Result<(), String> {
    ...
    let result = input.iter().fold((0, 0), |(depth, pos), direction| {
        match direction {
            &Direction::Forward(length) => (depth, pos + length),
            &Direction::Down(length) => (depth + length, pos),
            &Direction::Up(length) => (depth - length, pos),
        }
    });
